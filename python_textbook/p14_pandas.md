...
- index：索引不能存在可变数据，允许相同。不指定时，默认为从 0 开始依次递增的整数
- dtype：数据类型，如果没有指定，则会自动推断得出
- name：设置 Series 的名称，不能存在可变数据
...
- column：新插入的数据列的名字
- value：Series, or array-like，插入的数据
...
DataFrame.median(axis=0)	  返回指定轴的中位数
DataFrame.sum(axis=0)			返回指定轴的累加
DataFrame.prod(axis=0)		   返回指定轴的累积
